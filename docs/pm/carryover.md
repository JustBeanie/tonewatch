...
- ~~Linux `ca_file` unverified~~ **resolved 2026-09-11.** CI run 34673148889 (commit `45850f9`) ran `test_stream_https_certificate_verification` on ubuntu-latest and ubuntu-24.04-arm and both **PASSED** instead of skipping. On non-Windows that test re-raises if `tls_verify=1` with `ca_file=<test cert>` is refused, so Linux PyAV honours `ca_file`. The M8 Docker item below still applies.
- **HTTPS streams: Windows verified OK** (history below). PM probes 2026-09-11 corrected the S4-fix claim.
