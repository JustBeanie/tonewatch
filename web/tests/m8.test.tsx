...
describe("tones.cfg tone-set import", () => {
    it("renders the preview table and error rows", async () => {
        vi.spyOn(globalThis, "fetch").mockImplementation((input) =>
            String(input).includes("import/tones-cfg") ? json(preview) : json([]),
        );
        render(<App />);
        fireEvent.click(screen.getByRole("link", { name: "Tone sets" }));
        fireEvent.change(await screen.findByLabelText("tones.cfg file"), {
            target: { files: [new File(["[GoodPage]"], "tones.cfg")] },
        });
        expect(
            await screen.findByRole("heading", { name: "tones.cfg import preview" }),
        ).toBeInTheDocument();
    ...
    it("requires confirmation before replacing tone sets", async () => {
        const fetcher = vi
            ...
            .mockImplementation((input, init) =>
                String(input).includes("import/tones-cfg") && init?.method === "POST"
                    ? json(preview)
                    : json([]),
            );
        ...
        await waitFor(() =>
            expect(
                fetcher.mock.calls.filter((call) => String(call[0]).includes("import/tones-cfg")),
            ).toHaveLength(1),
        );
