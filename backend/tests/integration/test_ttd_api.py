"""API contract tests for TTD preview and apply."""
...
from tonewatch.api.app import MAX_TTD_IMPORT_BYTES, create_app
...
@pytest.mark.asyncio
async def test_ttd_api_validates_content_length(declared_length: str, expected_status: int) -> None:
...
@pytest.mark.asyncio
async def test_ttd_api_auth_csrf_preview_apply_and_replace() -> None:
    with TemporaryDirectory(ignore_cleanup_errors=True) as directory:
        root = Path(directory)
        ...
        async with (
            ...
            assert (await client.post("/api/import/ttd", content=source_text)).status_code == 401
            login = await client.post("/api/auth/login", json={"password": credential})
            csrf = login.json()["csrf_token"]
            csrf_headers = {"X-CSRF-Token": csrf}
            assert (await client.post("/api/import/ttd", content=source_text)).status_code == 403
            preview = await client.post(
                "/api/import/ttd", content=source_text, headers=csrf_headers
            )
            ...
            too_large = await client.post(
                "/api/import/ttd", content="x" * (256 * 1024 + 1), headers=bearer
            )
            assert too_large.status_code == 413
            applied = await client.post(
                "/api/import/ttd?apply=true&mode=replace",
            ...
            import_events = [item for item in audit_items if item["event_type"] == "ttd_import"]
...
@pytest.mark.asyncio
async def test_ttd_api_rejects_bad_utf8_and_missing_multipart_file() -> None:
    with TemporaryDirectory(ignore_cleanup_errors=True) as directory:
        root = Path(directory)
        store = ConfigStore(root)
        ...
        async with (
            ...
            token = (root / "api_token").read_text(encoding="ascii").strip()
            bearer = {"Authorization": f"Bearer {token}"}
            bad_utf8 = await client.post("/api/import/ttd", content=b"\xff", headers=bearer)
        ...
        async with (
            ...
            uploaded = await client.post(
                ...
                content=_three_chunks(
                    multipart_prefix_and_file,
                    b"\n",
                    b"x" * (MAX_TTD_IMPORT_BYTES + 64 * 1024 + 1),
                ),
                headers={
            ...
            assert not [
                item for item in audit.json()["items"] if item["event_type"] == "ttd_import"
            ]


@pytest.mark.asyncio
async def test_ttd_guard_covers_trailing_slash_and_root_path() -> None:
    with TemporaryDirectory(ignore_cleanup_errors=True) as directory:
        root = Path(directory)
        ConfigStore(root).save(AppConfig())
        app = create_app(Settings(data_dir=root))
        async with app.router.lifespan_context(app):
            ...
            async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
                unrelated = await client.post(
                    "/x/api/import/ttd",
                    content=_chunks(b"x" * (MAX_TTD_IMPORT_BYTES + 1)),
                    headers={
                        "Authorization": f"Bearer {token}",
                        "content-type": "text/plain",
                    },
